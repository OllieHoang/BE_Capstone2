...
function BookList() {
  ...
  const handleCallApiDelete = async () => {
    try {
      ...
      if (orders.length > 0) {
        toast.error("The product has been purchased and cannot be deleted!", {
          autoClose: 2000,
        });
        return;
      }
      ...
      setBookData((prevState) => {
        const newArray = prevState.books.filter(
          (item) => item._id !== bookDelete._id
        );
        return {
          ...prevState,
          books: newArray,
    ...
    <Row>
      <Modal size="lg" show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          ...
          <b>{bookDelete && bookDelete.name}</b>?
        </Modal.Body>
        <Modal.Footer>
